 ...
 */
public class TransportTruck extends Truck implements IPlatform, ITransporter<Car>, IObservablePosition {
     ...
     */
    TransportTruck(Point startPos, Direction dir) {
    ...
    public void gas(double amount) throws Exception {
        if (this.isInRestingState()) { //If ramp is raised => can run truck (not required of lab specific.)
            super.gas(amount);
        }
    }
